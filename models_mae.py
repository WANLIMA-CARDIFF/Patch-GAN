...
class MaskedAutoencoderViT(nn.Module):
    ...
    #     corr_recons = ((1 - disc_preds + 1e-8) * (1 - target)).sum() / ((1 - target).sum())
    #     # print(corr_orig)
    #     # print(corr_orig + corr_recons)
    #     return (corr_orig) + (corr_recons)

    def forward(self, imgs, GT, mask_ratio=0, split="train"):
        latent, mask, ids_restore = self.forward_encoder(imgs, mask_ratio)
        pred = self.forward_decoder(latent, ids_restore)  # [N, L, p*p*3]
        if split == "train":
            mask_all = torch.ones_like(mask, device=pred.device)
            if mask_ratio == 0:
                # _, mask_all, _ = self.random_masking(self.patch_embed(imgs), 1)
                loss = self.forward_loss(GT, pred, mask_all)
            else:
                loss = self.forward_loss(GT, pred, mask_all)

            GT_patched = self.patchify(GT)
            ...
            return self.unpatchify(pred), loss, mask, pred, GT_patched #, disc_loss, adv_loss
        else:
